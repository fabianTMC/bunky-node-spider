            ...
            if (eleText != null) {
              ...
              if (eleText.split('/').length == 3) {
                if (!start) {
                  ...
                  // we are on the next bunked day
                  days[currentDate] = dayBunked;
                  // remove the day
                  days[currentDate].splice(0, 1);
                  // remove the number of hours bunked
                  days[currentDate].splice(days[currentDate].length - 1, 1);
                  // clear the array and set the current date
                  dayBunked = [
                  ];
                  currentDate = eleText;
                }
                ...
                if (start) {
                  if (eleText == '') {
                    // this hour was attended
                    dayBunked.push({
                      'class': eleText,
                      'status': 'attended'
                    });
                  } else {
                    // chec if this is in the subject code name map && is not a weekday
                    if (!subjectCodeNameMap[eleText] && parseFloat(eleText) != eleText && dayNames.indexOf(eleText.toLowerCase()) == -1) {
      ...
      var r = request.post({
        ...
        if (err) {
          ...
          if ($('.news:contains("' + username + '")').filter(function () {
            ...
            failureCB && failureCB({
              'result': 'failure',
              'message': 'You have entered the wrong login details.'
            });
          }
        }
      });
